...
//        mCenterPoint=p;
    ...
    public TreasureView(Context context, PointF p, int x) {
        this(context, null);
        hasSetCen=true;
        mCenterPoint=p;
    }

    public TreasureView(Context context, AttributeSet attrs, int defStyle) {
    ...
    /*    SingleTouchView.this.setOnFocusChangeListener( new OnFocusChangeListener() {
        ...
        setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                TreasureView view = (TreasureView) v;
                view.setEditable(true);
            }
...
//            Log.e("parent", "onMesure"+Double.toString(parentWidth));
//            Log.e("parent", "onMesure"+Double.toString(parentHeight));
            mCenterPoint.set(parentWidth/2, parentHeight/2);
            hasSetCen=true;
            hasChange=true;
        }else if(null!=mViewGroup&&hasSetCen){
            if(!hasChange){
                int parentWidth=mViewGroup.getWidth();
                int parentHeight=mViewGroup.getHeight();
                mCenterPoint.set(mCenterPoint.x*parentWidth,mCenterPoint.y*parentHeight);
                hasChange=true;
            }
        }
        Log.e("parent", "onMesure"+Double.toString(mCenterPoint.x));
        Log.e("parent", "onMesure"+Double.toString(mCenterPoint.y));
    }
        ...
        switch (event.getAction() ) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mStatus == STATUS_ROTATE_ZOOM) {
                    ...
                    int newPaddingLeft = (int) (mCenterPoint.x - actualWidth /2);
                    int newPaddingTop = (int) (mCenterPoint.y - actualHeight/2);
                    int newPaddingRight = (int) (mCenterPoint.x + actualWidth /2);
                    int newPaddingBottom = (int) (mCenterPoint.y + actualHeight/2);

                    ViewGroup mViewGroup = (ViewGroup) getParent();
                    int parentWidth=10000;
                    int parentHeight=10000;
                    if(null != mViewGroup) {
                        parentWidth = mViewGroup.getWidth();
                        parentHeight = mViewGroup.getHeight();
                    }

                    if(newPaddingLeft<0&&(mCurMovePointF.x - mPreMovePointF.x)<0){

                    }else if(newPaddingRight>parentWidth&&(mCurMovePointF.x - mPreMovePointF.x)>0){

                    }else{
                        mCenterPoint.x += mCurMovePointF.x - mPreMovePointF.x;
                    }

                    if(newPaddingTop<0&&(mCurMovePointF.y - mPreMovePointF.y)<0){

                    }else if(newPaddingBottom>parentHeight&&(mCurMovePointF.y - mPreMovePointF.y)>0){
